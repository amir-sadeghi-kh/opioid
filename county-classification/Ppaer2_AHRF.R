# ==============================================================================
# Purpose: Prepare AHRF data which is county characteristics
# Author: Griffith et. al. (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC7881250/)
# Last modified: 06.05.2023
# Input: AHRF data for county characteristics 
# Output: AHRF cleaned
# ==============================================================================
...
setwd(paste0(path,"/AHRF/"))

## Load AHRF - update file paths as needed
arf2021 <- read_excel("2021-2022/arf2021-2022.xlsx", sheet = "Data")
arf2020 <- read_excel("2020-2021/arf2020-2021.xlsx", sheet = "Data")
arf2019 <- readRDS("2019-2020/arf2019-2020.RDS")
arf2018 <- readRDS("2018-2019/arf2018-2019.RDS")
arf2017 <- readRDS("2017-2018/arf2017-2018.RDS")
arf2016 <- readRDS("2016-2017/arf2016-2017.RDS")
arf2015 <- readRDS("2015-2016/arf2015-2016.RDS")
arf2014 <- readRDS("2014-2015/arf2014-2015.RDS")
arf2013 <- readRDS("2013-2014/arf2013-2014.RDS")
arf2012 <- readRDS("2012-2013/arf2012-2013.RDS")
arf2011 <- readRDS("2011-2012/arf2011-2012.RDS")
arf2009 <- readRDS("2009-2010/arf2009-2010.RDS")
arf2008 <- readRDS("2008/arf2008-2009.RDS")
arf2007 <- readRDS("2007/arf2007-2008.RDS")
arf2006 <- readRDS("2006/arf2006-2007.RDS")
...
keep <- c(
	...
	"f14480", # % aged 25+ w.o. high school diploma
	"f14481", # % aged 25+ w. high school diploma
...
# keep2021 <- c("f0978721", #  HPSA Code - Primary Care  05/21 | shortage in 1=Whole, 2=Part County        
#               "f0978720", #  HPSA Code - Primary Care  05/20
#               "f0978719", #  HPSA Code - Primary Care  05/19
#               "f0978718", #  HPSA Code - Primary Care  05/18
#               "f0978717", #  HPSA Code - Primary Care  05/17
#               "f1249221", #  HPSA Code - Primary Care  05/21       
#               "f1249220", #  HPSA Code - Primary Care  05/20
#               "f1249219", #  HPSA Code - Primary Care  05/19
#               "f1249218", #  HPSA Code - Primary Care  05/18
#               "f1249217" #  HPSA Code - Primary Care  05/17
#               )
...
arf2021.sub <- arf2021[,substr(names(arf2021),1,6) %in% keep]
arf2020.sub <- arf2020[,substr(names(arf2020),1,6) %in% keep]
arf2019.sub <- arf2019[,substr(names(arf2019),1,6) %in% keep]
...
# This is not a concern after 2017 (all have 5) 
...
## Merge years
comb <- merge(arf2021.sub, arf2020.sub, by="f00002")
comb <- merge(comb, arf2019.sub, by="f00002")
comb <- merge(comb, arf2018.sub, by="f00002")
comb <- merge(comb, arf2017.sub, by="f00002")
...
## Subset years
keep21 <- comb[,names(comb) %in% c("f00002","f04437","f0002013","f00010","f12424","f0453010","f0453810","f0454210") | substr(names(comb),7,8)=="21"]
keep20 <- comb[,names(comb) %in% c("f00002","f04437","f0002013","f00010","f12424","f0453010","f0453810","f0454210") | substr(names(comb),7,8)=="20"]
keep19 <- comb[,names(comb) %in% c("f00002","f04437","f0002013","f00010","f12424","f0453010","f0453810","f0454210") | substr(names(comb),7,8)=="19"]
...
names(keep21) <- substr(names(keep21), 1, 6)
names(keep20) <- substr(names(keep20), 1, 6)
names(keep19) <- substr(names(keep19), 1, 6)
...
ahrf <- smartbind(keep21, keep20, keep19, keep18,keep17, keep16, keep15, keep14, keep13, keep12, keep11, keep10, keep09, keep08, keep07, keep06)
...
write.csv(ahrf, "Cleaned AHRF 06-05-23 DIB.csv", row.names=FALSE)
saveRDS(ahrf, "Cleaned AHRF 06-05-23 DIB.RDS")
write_dta(ahrf, "Cleaned AHRF 06-05-23 DIB.dta")
